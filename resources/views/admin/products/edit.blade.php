...
@section('title', $product->name . ' - Edit Produk')
...
@section('content')
    <div class="layout-px-spacing">
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="post" enctype="multipart/form-data">
            ...
            <div class="row" id="cancel-row">
                        ...
                        <div class="form">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror" name="name"
                                    value="{{ old('name', $product->name) }}" id="name" maxlength="255"
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="stock" value="{{ old('stock', $product->stock) }}" id="stock"
                                    placeholder="Stok" class="form-control @error('stock') is-invalid @enderror"
                                    required="required">
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="description" id="description"
                                    ...
                                    placeholder="Deskripsikan produk kamu">{{ old('description', $product->description) }}</textarea>
                            ...
                            <div class="form-group">
                                <label for="is-available">
                                    <input type="checkbox" name="is_available" id="is-available" value="1" @if (old('is_available', $product->is_available) == 1) checked @endif>
                    ...
                    <div class="widget-content widget-content-area br-6 mt-3">
                        <div class="form">
                            ...
                            @if (old('specs') != null && is_array(old('specs')) && count(old('specs')) > 0)
                                ...
                                @endforeach
                            @else
                                @if (is_object($product->specifications) && count($product->specifications) > 0)
                                    @foreach ($product->specifications as $item)
                                        <div>
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 col-12">
                                                    <div class="form-group">
                                                        <label for="specs-value-{{ $loop->index }}">Isi
                                                            spesifikasi</label>
                                                        <input type="text" class="form-control"
                                                            id="specs-value-{{ $loop->index }}"
                                                            name="specs[{{ $loop->index }}][value]"
                    ...
                    <div class="widget-content widget-content-area br-6 mt-3">
                        <div class="form">
                            ...
                            @if (old('options') != null && is_array(old('options')) && count(old('options')) > 0)
                                ...
                                @endforeach
                            @else
                                @if (is_object($product->orderOptions) && count($product->orderOptions) > 0)
                                    @foreach ($product->orderOptions as $option)
                                        <div>
                                            <div class="row">
                                                ...
                                                <div class="col-md-6 col-12">
                                                    <div class="form-group">
                                                        <label for="option-value-{{ $loop->index }}">Pilihan
                                                            variasi</label>
                                                        <input type="text" class="form-control"
                                                            id="option-value-{{ $loop->index }}"
                                                            placeholder="Merah|Putih|Hijau"
                ...
                <div class="col-xl-4 col-lg-4 col-sm-12  layout-spacing">
                    <div class="widget-content widget-content-area br-6">
                        <h5 class="mb-3">
                            Foto Produk
                            <a href="#" class="btn btn-primary btn-sm btn-add-picture float-right"><i class="fa fa-plus"></i></a>
                        </h5>

                        @if (count($product->media) > 0)
                            <ul class="list-group pictures-group">
                                @foreach ($product->media as $media)
                                    <li class="list-group-item index-{{ $loop->index }}">
                                        <div class="row">
                                            <div class="col-2">
                                                <img src="{{ $media->getFullUrl() }}" alt="{{ $media->file_name }}"
                                                    class="img-fluid">
                                            </div>
                                            <div class="col-8">
                                            ...
                                            <div class="col-2">
                                                <a href="#" class="delete-image-link"
                                                    data-index="{{ $loop->index }}">&times;</a>
                                            </div>
                                        </div>
                                    </li>
                                @endforeach
                        ...
                        @else
                            <div class="custom-file-container" data-upload-id="images">
                                <label>Upload foto produk <a href="javascript:void(0)"
                                        ...
                                        title="Batalkan pilihan">&times;</a></label>
                                ...
                                <div>
                                    <label for="category-{{ $item->id }}">
                                        <input type="checkbox" name="categories[]" id="category-{{ $item->id }}"
                                            value="{{ $item->id }}" @if (in_array_r($item->id, $product->categories, 'id')) checked @endif>
                                        {{ $item->name }}
                                    </label>
                                </div>
                            @empty
                                ...
                                <div>
                                    <label for="brand-{{ $item->id }}">
                                        <input type="radio" name="brand_id" id="brand-{{ $item->id }}"
                                            value="{{ $item->id }}" @if ($product->brand_id == $item->id) checked @endif>
                                        {{ $item->name }}
                                    </label>
                                </div>
                            @empty
            ...
            <input type="hidden" name="_delete_picture_id" value="" class="delete-image-id">
...
@push('custom_js')
    ...
    <script>
        ...
        const deleteImageId = document.querySelector('.delete-image-id');
        ...
        deleteImageLinks.forEach((link) => {
            link.addEventListener('click', function(e) {
                ...
                let currentDeleted = deleteImageId.value;
                currentDeleted = (currentDeleted == '') ? `${index}` : `${currentDeleted},${index}`;

                deleteImageId.value = currentDeleted;

                const li = document.querySelector(`.index-${index}`);
                const loading = li.querySelector('.loading');
                li.classList.add('list-group-item-danger');
                loading.innerHTML = '<i class="fa fa-spin fa-spinner"></i> Menambah ke daftar hapus...';
        ...
        const pictureGroups = document.querySelector('.pictures-group');
        btnAddPicture.addEventListener('click', function (e) {
            ...
            li.innerHTML = `<input type="file" name="pictures[]" class="form-control">`;
            pictureGroups.appendChild(li);
        });
